					...
					<div class="row">
						...
						<div class="col-md-6">
							...
							<div class="card">
								...
								<form action="{{ route('producto.update', $p->id_producto ) }}" method="post">
									<div class="card-body">
										{!! csrf_field() !!}									
										{!! method_field('PUT') !!}
										<div class="form-group">
											<label for="text1">Nombre Producto</label>
											<input type="text" class="form-control" id="text1" name="nombre_producto" value="{{ $p->nombre_producto }}" placeholder="" required>
											{{-- <small id="emailHelp2" class="form-text text-muted">We'll never share your email with anyone else.</small> --}}
										</div>
										<div class="form-group">
											<label for="text1">Descripción</label>
											<input type="text" class="form-control" id="text1" name="descripcion" value="{{ $p->descripcion }}" placeholder="" required>
											{{-- <small id="emailHelp2" class="form-text text-muted">We'll never share your email with anyone else.</small> --}}
										</div>
										...
										<div class="form-group">
												...
												@foreach ($tipos as $t)	
													@if ($t->id_tipo_producto==$p->id_tipo_producto)
														<option selected value="{{ $t->id_tipo_producto }}">{{ $t->nombre_tipo_producto }}</option>
													@else
										...
										<div class="form-group">
												...
												@foreach ($familias as $f)	
													@if ($f->id_familia==$p->id_familia)
														<option selected value="{{ $f->id_familia }}">{{ $f->nombre_familia }}</option>
													@else
										...
										<div class="form-group">
											...
											<input type="number" class="form-control" id="text1" name="stock" value="{{ $p->stock }}" placeholder="" required>
											{{-- <small id="emailHelp2" class="form-text text-muted">We'll never share your email with anyone else.</small> --}}
										</div>	
										<div class="form-group">
											<label for="text1">Stock Critico</label>
											<input type="number" class="form-control" id="text1" name="stock_critico" value="{{ $p->stock_critico }}" placeholder="" required>
											{{-- <small id="emailHelp2" class="form-text text-muted">We'll never share your email with anyone else.</small> --}}
										</div>				
									</div>
									<div class="card-action">
										...
										<button type="submit" class="btn btn-success pull-right">Guardar Cambio</button>
									</div>
								</form>
							</div>							
						</div>	
